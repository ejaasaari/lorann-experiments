    ...
    for i in range(run_count):
        ...
        def single_query(v: numpy.array) -> Tuple[float, List[Tuple[int, float]]]:
            ...
            if prepared_queries:
                ...
                total = time.time() - start
            candidates = [
                (int(idx), float(dist)) for idx, dist in zip(candidates, metrics[distance].distance(v, X_train[candidates]))
            ]
        ...
        def batch_query(X: numpy.array) -> List[Tuple[float, List[Tuple[int, float]]]]:
            ...
            candidates = [
                [(int(idx), float(dist)) for idx, dist in zip(single_results, metrics[distance].distance(v, X_train[single_results]))]
