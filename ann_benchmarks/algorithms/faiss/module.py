...
class FaissIVFPQfs(Faiss):
    def __init__(self, metric, n_list, quantizer, dim_reduction):
        self._n_list = n_list
        self._metric = metric
    ...
    def fit(self, X):
        ...
        if self._dim_reduction > 0:
            factory_string = f"PCA{self._dim_reduction},IVF{self._n_list}"
            if self._quantizer:
                factory_string += "(SQfp16)"
            factory_string += f",PQ{self._dim_reduction // 2}x4fs"
        else:
            factory_string = f"IVF{self._n_list}"
            if self._quantizer:
                factory_string += "(SQfp16)"
            factory_string += f",PQ{d // 2}x4fs"
    ...
    def __str__(self):
        return "FaissIVFPQfs(n_list=%d, quantizer=%d, dim_reduction=%d, n_probe=%d, k_reorder=%d)" % (self._n_list, self._quantizer, self._dim_reduction, self._n_probe, self._k_reorder)